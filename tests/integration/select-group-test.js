...
moduleForComponent('select-group', '[Integration] SelectGroup component', {
  integration: true,
  teardown: function() {
    Ember.run( function() {
      select.destroy();
...
test('Test keyboard interaction', function(assert) {
  ...
  validateDropdownHidden = function(messageHidden) {
    assert.ok(Assertions.isHidden(find('.ember-select-results', $selectComponent)), messageHidden);
  };
